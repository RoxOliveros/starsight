...
class _RotateTheShapeScreenState extends State<RotateTheShapeScreen>
    with
        ...
        RoxieReactionMixin<RotateTheShapeScreen>,
        GameLoadingMixin,
        PuzzleAudioMixin,
        AiCameraMixin {
  @override
  ...
  void _startRound() {
    ...
    var choices = _objectPool
        .where((o) => !_usedItemIds.contains(o.id))
        .toList();
    if (choices.isEmpty) {
      _usedItemIds.clear();
      choices = _objectPool;
  ...
  Future<void> _saveDataAndShowWinDialog() async {
    ...
    PuzzleDatabaseService.saveGameData(
      gameId: 'puzzle_rotate_shape',
      activityName: 'Rotate The Shape',
    ...
    return GoodJobOverlay(
      ...
      onNext: () {
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder: (context) =>
                WhatGoesTogetherScreen(level: widget.level + 1),
          ),
        );
      },
      onRestart: () {
