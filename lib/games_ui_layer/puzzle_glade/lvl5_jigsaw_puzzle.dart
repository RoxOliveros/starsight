...
class _Lvl5JigsawPuzzleScreenState extends State<Lvl5JigsawPuzzleScreen>
  ...
  void _startRound() {
    ...
    _correctMapping = [0, 1, 2, 3]..shuffle(rng);

    _trayPieces = List.generate(4, (i) => _Piece(id: _correctMapping[i]))
      ..shuffle(rng);
      ...
      builder: (context, constraints) {
        ...
        return ClipRRect(
          ...
          child: OverflowBox(
            maxWidth: size * 2,
            maxHeight: size * 2,
            alignment: Alignment(col == 0 ? -1.0 : 1.0, row == 0 ? -1.0 : 1.0),
            child: Image.asset(
              'assets/images/objects/puzzle/$_currentObject.png',
              width: size * 2,
              height: size * 2,
  ...
  Widget _buildPieceContent(int pieceId, {double size = 64}) {
    // The crop quadrant must match the slot this piece belongs to,
    // not the piece's raw ID.
