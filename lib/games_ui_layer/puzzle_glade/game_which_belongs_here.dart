...
class _WhichBelongsHereScreenState extends State<WhichBelongsHereScreen>
  ...
  void _initAnimations() {
    ...
    _sceneScale = Tween<double>(
      begin: 0.92,
      end: 1.0,
    ).animate(CurvedAnimation(parent: _sceneEnterCtrl, curve: Curves.easeOut));
  ...
  Future<void> _onChoiceTapped(int index) async {
    ...
    if (isCorrect) {
      ...
      if (_round >= _kTotalRounds) {
        ...
        await PuzzleProgressService.instance
  ...
  Widget _buildWinOverlay() {
    return GoodJobOverlay(
      ...
      onNext: () {
        // Just an example; update when a subsequent level exists
