...
class Step2ChoiceScreen extends StatefulWidget {
  const Step2ChoiceScreen({super.key});
...
class _Step2ChoiceScreenState extends State<Step2ChoiceScreen>
  ...
  Future<void> _playQuestionAudio() async {
    await playAssetAudio(_player, 'assets/audio/lumi_town/level2/vo_step2_question.wav');
    await waitForAudio(_player);
    if (mounted) _iconEntranceCtrl.forward();
  }

  Future<void> _onCorrect() async {
    await playAssetAudio(_player, 'assets/audio/lumi_town/level2/vo_wash_start.wav');
    await waitForAudio(_player);
    if (!mounted) return;
    Navigator.of(context).pushReplacement(
      _fadeRoute(const Step2WashingScreen()),
    );
  }
